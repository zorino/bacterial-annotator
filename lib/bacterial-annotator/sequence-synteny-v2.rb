...
class SequenceSyntenyV2

  attr_reader :query_file, :subject_file, :aln_hits, :query_sequences, :subject_sequences

  def initialize root, options, query_file, subject_file, output_name, type, software
    ...
    @name = output_name
    @pidentity = @options[:pidentity]
    @min_coverage = @options[:pcoverage]
